...
async function fetchPokemonDetails() {
  ...
  const allowedPokemon = pokemonListResults.slice(
    0,
    pokemonLoadingLimit - countPokemonLoaded
  );
  const promises = allowedPokemon.map((pokemon) =>
...
function stopAutoload() {
  if (AUTOLOAD) {
    ...
    document.getElementById("autoload-toggle-label").innerHTML =
      "Autoload complete";
  }
  AUTOLOAD = false;
  stopSpinner("bar");
...
function findPokemonImage(pokemon) {
  ...
  return imageOf <= dreamworldSprites
